...
I was not able to find a way to get the combination of `regexpr()` and `regmatches()` to return more than the first instance.
...
together = unname(mapply(function(x,y){apply(y,1,function(z){substr(x,z[1],z[2])})},shopping_list,locations,SIMPLIFY=F))
...
Here I'm going to use the scenario from Section 14.4.2 in [R for Data Science](https://r4ds.had.co.nz/strings.html) instead of the one in the documentation because I find the nouns in sentences example easier to conceptualize than the phone number example.
...
together = unname(mapply(function(x,y){apply(y,1,function(z){substr(x,z[1],z[2])})},has_noun,locations,SIMPLIFY=F))

together

sepWord = lapply(together,strsplit," ")

first = unlist(lapply(sepWord,function(x){lapply(x,function(z){z[1]})}))
second = unlist(lapply(sepWord,function(x){lapply(x,function(z){z[2]})}))
...
splitRestrict <- function(string, pattern, n){
 ...
 toFinish = lapply(lapply(output, function(x){x[n:length(x)]}),function(x){paste(x,collapse=pattern)})
...
## base
cat(strwrap(thanks, width = 80)) ## match default
```
