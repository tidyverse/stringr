...
The following examples walk through the examples in the stringr documentation and reproduce the results in base R.
...
Locale distinctions are not made in base R.
...
## base
## take the output from str_locate_all for now
## see below for base R version of str_locate_all
## can use the source code for invert_match since it's in base

text_loc <- cbind(start = c(0L, num_loc[, "end"] + 1L), end = c(num_loc[, "start"] -
  1L, -1L))
text_loc
```
...
These are unique to stringr. However, many of the base R string functions have a `fixed` parameter that when true is equivalent to stringr's `fixed()` and when false is equivalent to stringr's `regex()`.
...
## stringr
strings <- c("abb", "a.b")
str_detect(strings, pattern) ## default is regex(pattern)
str_detect(strings, fixed(pattern))

## base
grepl(pattern, strings)
grepl(pattern, strings, fixed = T)
```
...
## base
mapply(function(x, y) {
  sapply(gregexpr(x, y, fixed = TRUE), function(i) sum(i > 0))
}, c("a", "b", "p", "p"), fruit) %>% unname()
...
Here is a trickier one. We need to wrangle the output of `gregexpr()` to match `str_extract_all()`.
...
getOrganized <- function(x) {
  ...
  lenEx <- attr(x, "match.length")

  end <- ifelse(lenEx == 0, start - 1, start + lenEx - 1)
  end[unlist(lapply(lenEx, is.null))] <- NA
  toReturn <- cbind(start, end)

  toReturn
}

locations <- lapply(gregexpr("[a-z]+", shopping_list), getOrganized)

together <- unname(mapply(function(x, y) {
  apply(y, 1, function(z) {
    substr(x, z[1], z[2])
  })
}, shopping_list, locations, SIMPLIFY = F))
...
do.call("rbind", mapply(function(x, y) {
  do.call("rbind", lapply(gregexpr(x, y), getOrganized))
}, c("a", "b", "p", "p"), fruit, SIMPLIFY = F))
```
...
lapply(gregexpr("p", fruit), getOrganized)
```
...
Here I'm going to use the scenario from Section 14.4.3 in [R for Data Science](https://r4ds.had.co.nz/strings.html) instead of the one in the documentation because I find the nouns in sentences example easier to conceptualize than the phone number example.
...
## base
matches <- regexec(noun, has_noun)
do.call("rbind", regmatches(has_noun, matches))
```

Now we need to organize the output of `gregexpr()` to match that of `str_match_all()`.
...
locations <- lapply(gregexpr(noun, has_noun), getOrganized)

together <- unname(mapply(function(x, y) {
  apply(y, 1, function(z) {
    substr(x, z[1], z[2])
  })
}, has_noun, locations, SIMPLIFY = F))

together

sepWord <- lapply(together, strsplit, " ")

first <- unlist(lapply(sepWord, function(x) {
  lapply(x, function(z) {
    z[1]
  })
}))
second <- unlist(lapply(sepWord, function(x) {
  lapply(x, function(z) {
    z[2]
  })
}))

unname(cbind(unlist(together), first, second))

## this matches the stringR version after a small adjustment
do.call("rbind", str_match_all(has_noun, noun))
```
...
ifelse(unlist(lapply(fruits, function(x) {
  grepl("b", x)
})), NA, fruits)
```
...
We need to do some formatting to make the output from `strsplilt()` match that of `str_split()`.
...
splitRestrict <- function(string, pattern, n) {
  output <- strsplit(string, pattern)

  test <- max(unlist(lapply(output, length)))
  if (n >= test) {
    return(output)
  } else {
    out <- vector("list", length(output))

    ## proof of concept
    ## should streamline to avoid a nested for loop
    for (j in 1:length(output)) {
      for (i in 1:(n - 1)) {
        out[[j]] <- c(out[[j]], output[[j]][i])
      }
    }


    toFinish <- lapply(lapply(output, function(x) {
      x[n:length(x)]
    }), function(x) {
      paste(x, collapse = pattern)
    })

    ## again both the for loop and the concatenating without prespecifying size are not ideal
    for (j in 1:length(output)) {
      out[[j]] <- c(out[[j]], toFinish[j])
    }
...
splitRestrict(fruits, " and ", 3)
splitRestrict(fruits, " and ", 2)
```
...
## base
do.call("rbind", splitRestrict(fruits, " and ", 3))

output <- splitRestrict(fruits, " and ", 4)
outputDF <- lapply(output, function(x) {
  as.data.frame(t(x), stringsAsFactors = F)
})

simplified <- plyr::rbind.fill(outputDF)
toShow <- as.matrix(simplified) %>% unname()
toShow[is.na(toShow)] <- ""
toShow
```
...
x
y <- paste(substr(x, 1, nchar(x) - 2), "GHIJ", substr(x, nchar(x), nchar(x)), sep = "")
y
...
`substr()` has less capability to handle NAs.
...
# substr(x4, 1, 2) <- NA ## no omit_na capability, Error: NA is invalid value
...
## base
# Using a complicated regular expression from
# https://stackoverflow.com/questions/25707647/merge-multiple-spaces-to-single-space-remove-trailing-leading-spaces


gsub("(?<=[\\s])\\s*|^\\s+|\\s+$", "", " String with trailing, middle,   and leading white space\t", perl = TRUE)
...
I'm not sure why these outputs don't match. `str_wrap()` is a wrapper around `stringi::stri_wrap()` which has defaults that are supposed to be compatible with `strwrap()`.
...
# New in stringr 1.4.0

## `str_starts`

*Detect the presence or absence of a pattern at the beginning of
a string*
...
*Detect the presence or absence of a pattern at the end of
a string*
...
## base
sepChar <- unlist(strsplit("the quick brown dog", ""))
paste(toupper(sepChar[1]), paste(sepChar[2:length(sepChar)], collapse = ""), sep = "")
```

Often the base R version is much more complicated with less tidy output. The stringr functions allow us to more easily manipulate strings.